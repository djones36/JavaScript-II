...
{"id":51,"first_name":"Bill","last_name":"John","email":"","shirt_size":"M","company_name":"Nova","donation":171}];
...
runners.forEach((bibNames) =>{
    fullName.push(`First Name: ${bibNames.first_name} Last Name: ${bibNames.last_name}`)
})
...
runners.filter((lShirt) => {
    if(lShirt.shirt_size === 'L'){
        largeShirts.push(`${lShirt.first_name}: ${lShirt.shirt_size}`)
    }
})

console.log(`Here are the Large Shirt people: ${largeShirts}`);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];

let totalDonations = runners.reduce((acc, donos) =>{
    // console.log(`This is donation: ${donos.donation}`)
    // console.log(`This is ACC: ${acc}`)
    return acc + donos.donation; 
}, 0)

ticketPriceTotal.push(totalDonations)

console.log(`Total Donations ticket price total: ${ticketPriceTotal}`);
